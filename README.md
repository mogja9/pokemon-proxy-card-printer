...
Faceted browse (set / language / type / promo + name search, sortable by newest/oldest/set, with a 24/48/96 per-page control, a one-click Clear when any filter is active, a jump-to-page box on the pager, and a context-aware empty state that suggests which filters to loosen), card detail with a
